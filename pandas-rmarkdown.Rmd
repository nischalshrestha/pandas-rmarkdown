...
Because we had repiting values for the `time_hour` column, one last trick is to use `kableExtra::collapse_rows(columns = 1, valign = "top")` to white out repititions of the first column. Behold, the final output (with rows chopped for brevity):
...
I hope you have enjoyed this weird tutorial of how to pretty print Python dataframes with RMarkdown. If you have any suggestions or ideas (or spot mistakes), please let me know! One can go even further with this by adding in column types to the columns, and maybe even making an interactive version with `reactable`.
